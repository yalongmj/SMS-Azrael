				...
				<table class="table table-bordered wlsm-student-payment-history-table wlsm-w-100">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a class="wlsm-<?php echo esc_attr( $action_for ); ?>-print-invoice-payment wlsm-ml-1" data-invoice-payment="<?php echo esc_attr( $row->ID ); ?>" data-student="<?php echo esc_attr( $row->student_id ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( $action_for . '-print-invoice-payment-' . $row->ID ) ); ?>" href="#" data-message-title="<?php printf( /* translators: %s: receipt number */ esc_attr__( 'Payment Receipt - %s', 'school-management' ), esc_attr( WLSM_M_Invoice::get_receipt_number_text( $row->receipt_number ) ) ); ?>" data-close="<?php echo esc_attr__( 'Close', 'school-management' ); ?>"> <?php esc_html_e( 'Print', 'school-management' ); ?>
								</a>
							</td>
						</tr>
							<?php
