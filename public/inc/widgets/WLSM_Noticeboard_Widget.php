...
class WLSM_Noticeboard_Widget extends WP_Widget {
	public function __construct() {
		$widget_options = array(
			...
			'description' => esc_html__( 'Display school notices.', 'school-management' ),
		);
		parent::__construct( 'wlsm_noticeboard_widget', esc_html__( 'School Noticeboard', 'school-management' ), $widget_options );
	}
	...
	public function widget( $args, $instance ) {
		...
		$school_id          = isset( $instance['school_id'] ) ? absint( $instance['school_id'] ) : 0;
	...
	public function form( $instance ) {
		$schools = WLSM_M_School::get_schools();

		$fields = array(
			'title'              => esc_html__( 'Noticeboard', 'school-management' ),
			'school_id'          => '',
			'number_of_notices'  => 6,
		...
		$school_id          = ! empty( $instance['school_id'] ) ? $instance['school_id'] : '';
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'school_id' ) ); ?>"><?php esc_html_e( 'School', 'school-management' ); ?>:</label><br>
			<select class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'school_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'school_id' ) ); ?>">
				<option value=""><?php esc_html_e( 'Select School', 'school-management' ); ?></option>
				<?php foreach ( $schools as $value ) { ?>
				<option <?php selected( $school_id, $value->ID, true ); ?> value="<?php echo esc_attr( $value->ID ); ?>">
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'school-management' ); ?>:</label><br>
			<input class="widefat" type="text" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $title ); ?>">
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_notices' ) ); ?>"><?php esc_html_e( 'Number of Notices', 'school-management' ); ?>:</label><br>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'animation_interval' ) ); ?>"><?php esc_html_e( 'Animation Interval (in seconds)', 'school-management' ); ?>:</label><br>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'max_height' ) ); ?>"><?php esc_html_e( 'Maximum Height (in pixels)', 'school-management' ); ?>:</label><br>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'min_height' ) ); ?>"><?php esc_html_e( 'Minimum Height (in pixels)', 'school-management' ); ?>:</label><br>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['school_id']          = absint( $new_instance['school_id'] );
